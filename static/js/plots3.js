...
d3.json("http://127.0.0.1:5000/api/v1.0/date").then((importedData) => {
  ...
  Canada = [];
    ...
    var filterData = pieData.forEach( row => {
    if (row[1] === 2019) {
      if (row[0] === 'Alberta'){
        if(row[2] != 'Total all types of electricity generation'){
          if (row[2] === 'Hydraulic turbine' || row[2] === 'Wind power turbine' || row[2] === 'Solar' || row[2] === 'Tidal power turbine'){
                rAlberta.push(row[3]);}
          else if (row[2] === 'Total electricity production from combustible fuels' ||row[2] === 'Nuclear steam turbine' ) {
          nrAlberta.push(row[3])
        }
        }
      }
        ...
        if(row[2] != 'Total all types of electricity generation'){
          if (row[2] === 'Hydraulic turbine' || row[2] === 'Wind power turbine' || row[2] === 'Solar' || row[2] === 'Tidal power turbine'){
                rNewfoundland.push(row[3]);}
          else if (row[2] === 'Total electricity production from combustible fuels' ||row[2] === 'Nuclear steam turbine' ) {
          nrNewfoundland.push(row[3])
        }
        }
      }
      else if (row[0] === 'Canada'){
        if(row[2] != 'Total all types of electricity generation'){
          if (row[2] === 'Hydraulic turbine' || row[2] === 'Wind power turbine' || row[2] === 'Solar' || row[2] === 'Tidal power turbine'){
                rCanada.push(row[3]);}
          else if (row[2] === 'Total electricity production from combustible fuels' ||row[2] === 'Nuclear steam turbine' ) {
    ...
    var ralberatSum = rAlberta.reduce(function(a, b){
      return a + b;
    }, 0);
    var rbcSum = rBc.reduce(function(a, b){
      return a + b;
    }, 0);
    var rnewfoundlandSum = rNewfoundland.reduce(function(a, b){
      return a + b;
    }, 0); 
    var ronSum = rOntario.reduce(function(a, b){
      return a + b;
    }, 0);
    var rqSum = rQuebec.reduce(function(a, b){
      return a + b;
    }, 0);
    var rcanadaSum = rCanada.reduce(function(a, b){
      return a + b;
    }, 0);
  
    var nralberatSum = nrAlberta.reduce(function(a, b){
      return a + b;
    }, 0);
    var nrbcSum = nrBc.reduce(function(a, b){
      return a + b;
    }, 0);
    var nrnewfoundlandSum = nrNewfoundland.reduce(function(a, b){
      return a + b;
    }, 0); 
    var nronSum = nrOntario.reduce(function(a, b){
      return a + b;
    }, 0);
    var nrqSum = nrQuebec.reduce(function(a, b){
      return a + b;
    }, 0);
    var nrcanadaSum = nrCanada.reduce(function(a, b){
      return a + b;
    }, 0);

  var allLabels = ['Non Renewable', 'Renewable'];

  var trace1 = {
    values: [nralberatSum, ralberatSum],
    labels: allLabels,
    type: 'pie',
    name: 'Alberta',
    hoverinfo: 'label+percent+name',
    marker: {
      colors: ['rgb(128, 0, 0)', 'rgb(0, 102, 0)']
    },
    
  };
  var trace2 = {
    values: [nrbcSum, rbcSum],
    labels: allLabels,
    type: 'pie',
  ...
  var trace8 = {
    values: [nrnewfoundlandSum, rnewfoundlandSum],
    labels: allLabels,
    type: 'pie',
  ...
  // var data = [trace14, trace1, trace2, trace3, trace4, trace5, trace6, trace7, trace8, trace9, trace10, trace11, trace12, trace13]
  ...
  Plotly.newPlot('plot5' , [trace14], layout)
  Plotly.newPlot('plot6' , [trace1], layout2)
  Plotly.newPlot('plot7' , [trace2], layout2)
  Plotly.newPlot('plot8' , [trace3], layout2)
  Plotly.newPlot('plot9' , [trace8], layout2)
  Plotly.newPlot('plot10' , [trace9], layout2)
